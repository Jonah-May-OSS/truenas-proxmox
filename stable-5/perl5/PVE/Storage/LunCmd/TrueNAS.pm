...
sub run_lun_command {
    ...
    if(!defined($scfg->{'truenas_user'}) || !defined($scfg->{'truenas_password'})) {
        die "Undefined truenas_user and/or truenas_password.";
    }

    syslog("info","TrueNAS::lun_command : $method(@params)");
    ...
    syslog("error","TrueNAS::lun_command : unknown method $method");
    return undef;
}
...
sub run_list_lu {
    ...
    foreach my $lun (@$luns) {
        if ($lun->{'iscsi_target_extent_path'} =~ /^$object$/) {
            ...
            syslog("info","TrueNAS::list_lu($object):$result_value_type : lun found $result");
            last;
        }
    }
    if(!defined($result)) {
        syslog("info","TrueNAS::list_lu($object):$result_value_type : lun not found");
    }
...
sub run_list_extent {
    ...
    foreach my $lun (@$luns) {
        if ($lun->{'iscsi_target_extent_path'} =~ /^$object$/) {
            ...
            syslog("info","TrueNAS::list_extent($object): naa found $result");
            last;
        }
    }
    if (!defined($result)) {
        syslog("info","TrueNAS::list_extent($object): naa not found");
    }
...
sub run_create_lu {
    ...
    if (defined($link)) {
       syslog("info","TrueNAS::create_lu(lun_path=$lun_path, lun_id=$lun_id) : sucessfull");
    } else {
...
sub run_delete_lu {
    ...
    if($remove_link == 1 && $remove_extent == 1) {
        syslog("info","TrueNAS::delete_lu(lun_path=$lun_path) : sucessfull");
    } else {
...
sub truenas_api_call {
    ...
    $client->addHeader('Authorization' , 'Basic ' . encode_base64($scfg->{truenas_user} . ':' . $scfg->{truenas_password}));
    ...
    if ($code != 200) {
        truenas_api_log_error($client, "truenas_api_call");
        die "Unable to connect to the TrueNAS API service at '" . $apihost . "' using the '" . $scheme . "' protocol";
    }
    syslog("info","TrueNAS::api_call : setup : sucessfull");
    if ($method eq 'GET') {
        $client->GET($path);
    }
    if ($method eq 'DELETE') {
...
sub truenas_api_log_error {
    my ($client, $method) = @_;
    syslog("info","[ERROR]TrueNAS::API::" . $method . " : Response code: " . $client->responseCode());
    syslog("info","[ERROR]TrueNAS::API::" . $method . " : Response content: " . $client->responseContent());
    return 1;
}
...
sub truenas_iscsi_get_globalconfiguration {
    my ($scfg) = @_;
    my $client = truenas_api_call($scfg, 'GET', "/api/v1.0/services/iscsi/globalconfiguration/", undef);
    my $code = $client->responseCode();

    if ($code == 200) {
        my $result = decode_json($client->responseContent());
        syslog("info","TrueNAS::API::get_globalconfig : target_basename=" . $result->{'iscsi_basename'});
        return $result;
    } else {
        truenas_api_log_error($client, "get_globalconfig");
        return undef;
    }
}
...
sub truenas_iscsi_get_extent {
    my ($scfg) = @_;
    my $client = truenas_api_call($scfg, 'GET', "/api/v1.0/services/iscsi/extent/?limit=0", undef);

    my $code = $client->responseCode();
    if ($code == 200) {
        my $result = decode_json($client->responseContent());
        syslog("info","TrueNAS::API::get_extent : sucessfull");
        return $result;
    } else {
        truenas_api_log_error($client, "get_extent");
        return undef;
    }
}
...
sub truenas_iscsi_create_extent {
    ...
    my $client = truenas_api_call($scfg, 'POST', "/api/v1.0/services/iscsi/extent/", $request);
    my $code = $client->responseCode();
    if ($code == 201) {
        my $result = decode_json($client->responseContent());
        syslog("info", "TrueNAS::API::create_extent(lun_path=" . $result->{'iscsi_target_extent_path'} . ") : sucessfull");
        return $result;
    } else {
        truenas_api_log_error($client, "create_extent");
        return undef;
    }
}
...
sub truenas_iscsi_remove_extent {
    ...
    my $client = truenas_api_call($scfg, 'DELETE', "/api/v1.0/services/iscsi/extent/$extent_id/", undef);
    my $code = $client->responseCode();
    if ($code == 204) {
        syslog("info","TrueNAS::API::remove_extent(extent_id=$extent_id) : sucessfull");
        return 1;
    } else {
        truenas_api_log_error($client, "remove_extent");
        return 0;
    }
}
...
sub truenas_iscsi_get_target {
    ...
    my $client = truenas_api_call($scfg, 'GET', "/api/v1.0/services/iscsi/target/?limit=0", undef);
    my $code = $client->responseCode();
    if ($code == 200) {
        my $result = decode_json($client->responseContent());
        syslog("info","TrueNAS::API::get_target() : sucessfull");
        return $result;
    } else {
        truenas_api_log_error($client, "get_target");
        return undef;
    }
}
...
sub truenas_iscsi_get_target_to_extent {
    ...
    my $client = truenas_api_call($scfg, 'GET', "/api/v1.0/services/iscsi/targettoextent/?limit=0", undef);
    my $code = $client->responseCode();
    if ($code == 200) {
        my $result = decode_json($client->responseContent());
        syslog("info","TrueNAS::API::get_target_to_extent() : sucessfull");
        ...
        foreach my $item (@$result) {
            if (!defined($item->{'iscsi_lunid'})) {
                $item->{'iscsi_lunid'} = 0;
                syslog("info", "TrueNAS::API::get_target_to_extent() : change undef iscsi_lunid to 0");
            }
        }
        return $result;
    } else {
        truenas_api_log_error($client, "get_target_to_extent");
        return undef;
    }
}
...
sub truenas_iscsi_create_target_to_extent {
    ...
    my $client = truenas_api_call($scfg, 'POST', "/api/v1.0/services/iscsi/targettoextent/", $request);
    my $code = $client->responseCode();
    if ($code == 201) {
        my $result = decode_json($client->responseContent());
        syslog("info","TrueNAS::API::create_target_to_extent(target_id=$target_id, extent_id=$extent_id, lun_id=$lun_id) : sucessfull");
        return $result;
    } else {
        truenas_api_log_error($client, "create_target_to_extent");
        return undef;
    }
}
...
sub truenas_iscsi_remove_target_to_extent {
    ...
    my $client = truenas_api_call($scfg, 'DELETE', "/api/v1.0/services/iscsi/targettoextent/$link_id/", undef);
    my $code = $client->responseCode();
    if ($code == 204) {
        syslog("info","TrueNAS::API::remove_target_to_extent(link_id=$link_id) : sucessfull");
        return 1;
    } else {
        truenas_api_log_error($client, "remove_target_to_extent");
        return 0;
    }
}
...
sub truenas_list_lu {
                ...
                foreach my $node (@$extents) {
                    if($node->{'id'} == $item->{'iscsi_extent'}) {
                        if ($item->{'iscsi_lunid'} =~ /(\d+)/) {
                            ...
                            syslog("info", "TrueNAS::API::truenas_list_lu : iscsi_lunid did not pass tainted testing");
                            next;
                        }
    ...
    syslog("info", "TrueNAS::API::truenas_list_lu : sucessfull");
    return \@luns;
}
...
sub truenas_get_first_available_lunid {
    ...
    syslog("info", "TrueNAS::API::truenas_get_first_available_lunid : return $lun_id");
    return $lun_id;
}
...
sub truenas_get_targetid {
    ...
    my $global    = truenas_iscsi_get_globalconfiguration($scfg);
    my $targets   = truenas_iscsi_get_target($scfg);
    my $target_id = undef;
